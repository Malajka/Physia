...
describe("GET /api/sessions/[session_id]/exercises", () => {
  ...
  const mockExercises: ExerciseDto[] = [{ id: 1, name: "Push Up", created_at: "2024-01-01T00:00:00Z", session_id: 42 }];
  ...
  it("should return 200 with exercises for a valid request", async () => {
    mockedGetExercises.mockResolvedValue({ exercises: mockExercises, error: null });
    const context = createMockContext({ session_id: "42" });

    await GET(context, mockUserId);
