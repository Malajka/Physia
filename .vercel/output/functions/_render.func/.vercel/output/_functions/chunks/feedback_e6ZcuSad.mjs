...
async function getFeedbackForSession(supabase, userId, sessionId) {
  const { data, error } = await supabase.from("feedback_ratings").select("rating, rated_at").eq("session_id", sessionId).eq("user_id", userId).maybeSingle();
  if (error) {
...
async function upsertFeedback(supabase, userId, sessionId, rating) {
  ...
  const { data, error } = await supabase.from("feedback_ratings").upsert({ session_id: sessionId, user_id: userId, rating, rated_at }, { onConflict: "session_id,user_id" }).select().single();
  if (error || !data) {
...
const FeedbackParamsSchema = z.object({
  session_id: z.coerce.number({ invalid_type_error: "session_id must be a number" }).int({ message: "session_id must be an integer" }).positive({ message: "session_id must be a positive integer" })
});
...
export { FeedbackParamsSchema as F, GET as G, POST as P, _page as _, FeedbackBodySchema as a, getFeedbackForSession as g, upsertFeedback as u };
