import {
  u as ROUTE_TYPE_HEADER,
  v as REROUTE_DIRECTIVE_HEADER,
  D as DEFAULT_404_COMPONENT,
  A as AstroError,
  w as ActionNotFoundError,
  x as clientAddressSymbol,
  y as LocalsNotAnObject,
  z as REROUTABLE_STATUS_CODES,
  B as responseSentSymbol,
} from "./astro/server_CfAXeihZ.mjs";
import { bold, red, yellow, dim, blue } from "kleur/colors";
import "clsx";
import "cookie";
import { D as DEFAULT_404_ROUTE, d as default404Instance, e as ensure404Route } from "./astro-designed-error-pages_Cv7EM1T7.mjs";
...
import {
  ...
  d as normalizeTheLocale,
  e as defineMiddleware,
  S as SERVER_ISLAND_COMPONENT,
  f as SERVER_ISLAND_ROUTE,
  g as createEndpoint,
  R as RouteCache,
  s as sequence,
  h as findRouteToRewrite,
  m as matchRoute,
  j as RenderContext,
  P as PERSIST_SYMBOL,
  k as getSetCookiesFromResponse,
} from "./index_C_bCMJXd.mjs";
import { N as NOOP_MIDDLEWARE_FN } from "./noop-middleware_MfYCK3Rr.mjs";
...
class Pipeline {
  constructor(
    ...
    if (i18n?.strategy !== "manual") {
      this.internalMiddleware.push(createI18nMiddleware(i18n, manifest.base, manifest.trailingSlash, manifest.buildFormat));
    }
  }
  internalMiddleware;
...
class AppPipeline extends Pipeline {
  #manifestData;
  static create(manifestData, { logger, manifest, runtimeMode, renderers, resolve, serverLike, streaming, defaultRoutes }) {
...
class App {
  ...
  async render(request, renderOptions) {
    ...
    if (!routeData) {
      routeData = this.#manifestData.routes.find((route) => route.component === "404.astro" || route.component === DEFAULT_404_COMPONENT);
    }
    if (!routeData) {
   ...
   */
  async #renderError(request, { locals, status, response: originalResponse, skipMiddleware = false, error, clientAddress }) {
