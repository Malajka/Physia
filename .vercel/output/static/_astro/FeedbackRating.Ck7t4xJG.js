import { j as t } from "./jsx-runtime.BMmiHB9I.js";
import { B as b } from "./SubmitButton.D_2OGRIf.js";
...
async function h(a, e) {
  const s = await fetch(`/api/sessions/${a}/feedback`, { method: "POST", headers: p, body: JSON.stringify({ rating: e }) }),
    o = await s.json();
  if (!s.ok) throw new Error(o.error || "Failed to submit feedback");
  return o;
}
const E = ({ sessionId: a }) => {
  ...
  n.useEffect(() => {
    ...
    async function r() {
      try {
        ...
        i && f(c instanceof Error ? c.message : String(c));
      } finally {
        i && k(!1);
      }
    }
